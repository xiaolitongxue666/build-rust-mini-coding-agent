...
9) **记忆 SSOT** — 本文件。/summary-memory 走 summary-project-memory；不写 ~/.claude-mem。备份 .project-memory-backups/（gitignore，留 2 份）。不自动改 AGENTS.md。Windows 上 draft 用 .local/memory-draft.md，不要依赖 /tmp。
